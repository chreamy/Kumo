...
let desc = "Get top tokens by market cap from Luminex";

const LUMINEX_API_URL =
  "https://api.luminex.io/spark/pulse?page=1&limit=100&filters=%7B%22category%22%3A%22migrated%22%2C%22token_b_pubkeys%22%3A%5B%22020202020202020202020202020202020202020202020202020202020202020202%22%5D%2C%22include_hostnames%22%3A%5B%22luminex%22%2C%22utxofun%22%5D%2C%22sort_by%22%3A%22market_cap_usd%22%2C%22order%22%3A%22desc%22%7D";
  ...
  try {
    ...
    const tokenList = topTokens
      .map((token, index) => {
        ...
        entry += `\n💰 $${priceUsd.toFixed(
          8
        )} • ${changeEmoji} ${changeText} • 📊 ${marketCapFormatted} • 👥 ${holders.toLocaleString()}`;
